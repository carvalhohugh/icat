...
import { ArrowLeft, ArrowRight, Check, Send, RotateCcw, User, Phone, Heart, Shield, GraduationCap, Wrench, Bus, Droplets, Wifi, TreePine, Building2, Utensils, ClipboardList } from 'lucide-react';
        ...
        <div className="flex-1 p-6 flex flex-col">
          ...
          {step < 3 && (
            <div className="flex gap-2 mb-8 mt-2">
              {[0, 1, 2].map(i => (
                <div key={i} className={`flex-1 h-2 rounded-full transition-all duration-300 ${step >= i ? 'bg-icat-green shadow-sm' : 'bg-gray-100'}`}></div>
              ))}
            </div>
          )}
          ...
          {step === 0 && (
            <div className="space-y-6 animate-in fade-in flex-1 flex flex-col">
              <div className="text-center">
                ...
                <p className="text-gray-500 mt-2 text-sm">Precisamos do seu CPF para validar a autenticidade da pesquisa.</p>
              </div>
              ...
              <div className="space-y-5 flex-1">
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1.5 ml-1">CPF (Obrigatório)</label>
              ...
              <button onClick={() => { if (nome.trim() && cpf.trim()) setStep(1); }}
          ...
          {step === 1 && (
            <div className="space-y-6 animate-in fade-in flex-1 flex flex-col">
              <div className="text-center">
                ...
                <p className="text-gray-500 mt-2 text-sm">Selecione apenas uma opção.</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 flex-1">
                ...
                {!pesquisa.induzida && (
                  <button onClick={() => setSelecionado(pesquisa.opcoes.length)}
                    className={`relative rounded-2xl p-5 text-left transition-all duration-300 border-2 border-dashed ${
              ...
              {selecionado === pesquisa.opcoes.length && !pesquisa.induzida && (
                <div className="animate-in slide-in-from-bottom-2">
                  <input type="text" value={outroNome} onChange={e => setOutroNome(e.target.value)}
                    className="w-full px-4 py-4 border-2 border-gray-200 rounded-xl focus:border-icat-green focus:ring-4 focus:ring-icat-green/10 outline-none transition-all font-bold text-gray-900 text-center"
                    placeholder="Digite sua resposta..." autoFocus />
                </div>
              )}

              <div className="flex gap-3 mt-8">
                <button onClick={() => setStep(0)} className="w-14 h-14 flex items-center justify-center rounded-xl border-2 border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors">
                ...
                <button onClick={() => { if (selecionado !== null) setStep(2); }}
                  ...
                  className="flex-1 bg-gray-900 text-white font-bold rounded-xl hover:bg-gray-800 disabled:opacity-40 transition-colors flex items-center justify-center gap-2 shadow-xl shadow-gray-900/20">
          ...
          {step === 2 && (
            <div className="space-y-6 animate-in fade-in flex-1 flex flex-col">
              <div className="text-center">
                <h2 className="text-2xl font-bold text-gray-900">Confirme seu voto</h2>
                <p className="text-gray-500 mt-2 text-sm">Quase lá! Revise as informações antes de concluir.</p>
              </div>

              <div className="bg-gray-50 rounded-3xl p-6 border border-gray-100 flex-1">
                <div className="space-y-4">
                  <div className="bg-white p-4 rounded-2xl shadow-sm">
                    <span className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Entrevistado</span>
                    <span className="block text-base font-bold text-gray-900">{nome}</span>
                    <span className="block text-sm text-gray-500 mt-0.5">CPF: {cpf}</span>
                  </div>
              ...
              <div className="flex gap-3 mt-8">
                <button onClick={() => setStep(1)} className="w-14 h-14 flex items-center justify-center rounded-xl border-2 border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors">
                  <ArrowLeft className="w-5 h-5" />
                </button>
                <button onClick={() => setStep(3)}
                  className="flex-1 bg-gradient-to-r from-icat-blue to-icat-green text-white font-bold rounded-xl hover:opacity-90 flex items-center justify-center gap-2 shadow-xl shadow-green-500/20 text-lg">
          ...
          {step === 3 && (
            <div className="flex flex-col items-center justify-center text-center py-12 animate-in fade-in flex-1">
              ...
              <p className="text-gray-500 mb-8 max-w-[250px] leading-relaxed">
                Sua resposta foi registrada com sucesso e ajudará a construir um futuro melhor.
              </p>
