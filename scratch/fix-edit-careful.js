...
// 2. Fix the handleSavePesquisa function to use editandoId
const handleSavePesquisaStr = `const handleSavePesquisa = () => {
...
// replace the old handleSavePesquisa
content = content.replace(/const handleSavePesquisa = \(\) => \{[\s\S]*?setIsModalOpen\(false\);\s*\};/, handleSavePesquisaStr);

// add updatePesquisa to imports
...
// 3. Fix the "Nova Pesquisa" button to clear editandoId
content = content.replace(
    /onClick=\{\(\) => setIsModalOpen\(true\)\}/,
    `onClick={() => { setEditandoId(null); setNovaPesquisa({ nome: '', tipo: 'Intenção de Voto', induzida: true, multiSelect: false, metaDiaria: 50, metaEntrevistas: 1000, exigeMorador: false, idadeMinima: 16 }); setNovasOpcoes([{nome: '', partido: ''}, {nome: '', partido: ''}]); setEntrevSelecionados([]); setIsModalOpen(true); }}`
);

// 4. Update the card onClick logic
...
console.log('Fixed edit mode carefully!');
